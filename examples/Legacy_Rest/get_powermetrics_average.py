...
def get_powermetrics_average(restobj):
    resource_instances = get_resource_directory(restobj)
    if resource_instances:
        #Get URI from resource directory
        for instance in resource_instances:
            if "PowerMetrics." in instance.Type:
                power_path = instance.href
                break

    response = restobj.get(power_path)
...
if __name__ == "__main__":
    ...
    get_powermetrics_average(REST_OBJ)
    REST_OBJ.logout()
