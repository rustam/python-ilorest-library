...
def mount_virtual_media_iso(restobj, iso_url, boot_on_next_server_reset):
    ...
    rsp = restobj.get(rsp.dict["links"]["VirtualMedia"]["href"])

    for vmlink in rsp.dict["links"]["Member"]:
        response = restobj.get(vmlink["href"])
        ...
        if response.status == 200 and "DVD" in response.dict["MediaTypes"]:
            ...
            if (iso_url is not None and  boot_on_next_server_reset is not None):
                ...
                response = restobj.patch(vmlink["href"], body)
...
if __name__ == "__main__":
    ...
    mount_virtual_media_iso(REST_OBJ, "http://10.0.0.100/test.iso", True)
    REST_OBJ.logout()
