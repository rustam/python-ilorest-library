...
def get_csr(restobj, filename):
    resource_instances = get_resource_directory(restobj)
    if resource_instances:
        #Get URI from resource directory
        for instance in resource_instances:
            if "HpHttpsCert." in instance.Type:
                https_path = instance.href
                break

    response = restobj.get(https_path)
    try:
        ...
        sys.stdout.write("\tCSR Data saved successfully as "+ filename + "\n")
    except KeyError:
        sys.stdout.write("\tCSR cannot be accessed right now, please try again later")
